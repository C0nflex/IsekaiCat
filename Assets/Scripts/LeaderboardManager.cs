...
public class LeaderboardManager : MonoBehaviour
{
    ...
    public void SubmitScore() => SetLeaderboardEntry(inputName.text, GameManager.Instance.Record);

    public void UpdateScore() => LeaderboardRefrence.GetPersonalEntry((entry) =>{
        if(entry.Rank == 0) //no entry uploaded
            SetLeaderboardEntry("guest" + Random.Range(1,1000).ToString(), GameManager.Instance.Record);
        else
            SetLeaderboardEntry(entry.Username, GameManager.Instance.Record);
    });
} */
